...
from . import CellEditor, OLVEvent
    ...
    * useAlternateBackColors
    ...
    def _InitializeCheckBoxImages(self):
        ...
        def _makeBitmap(state, size):
            bitmap = wx.Bitmap(size, size) if "phoenix" in wx.PlatformInfo else wx.EmptyBitmap(size, size)
            dc = wx.MemoryDC(bitmap)
            dc.Clear()

            # On Linux, the Renderer draws the checkbox too low
            yOrigin = -1 if wx.Platform == "__WXGTK__" else 0
    ...
    def InstallCheckStateColumn(self, column):
        ...
        if self.smallImageList is None or not self.smallImageList.HasName(ObjectListView.NAME_CHECKED_IMAGE):
    ...
    def _InsertUpdateItem(self, listItem, index, modelObject, isInsert):
        ...
        for iCol in range(1, len(self.columns)):
            if "phoenix" in wx.PlatformInfo:
                self.SetItem(index, iCol, self.GetStringValueAt(modelObject, iCol), self.GetImageAt(modelObject, iCol))
            else:
                self.SetStringItem(
                    index, iCol, self.GetStringValueAt(modelObject, iCol), self.GetImageAt(modelObject, iCol)
                )
    ...
    def _ResizeSpaceFillingColumns(self):
        ...
        if True not in {x.isSpaceFilling for x in self.columns}:
            return
    ...
    def _HandleChar(self, evt):
        ...
        if not self.IsCellEditing() and self._HandleTypingEvent(evt):
            return None
    ...
    def _HandleTypingEvent(self, evt):
        ...
        if uniKey == 0:
            ...
            uniChar = six.unichr(uniKey) if isinstance(uniKey, int) else uniKey
    ...
    def _HandleTabKey(self, isShiftDown):
        ...
        if self.HasFlag(wx.LC_REPORT):
            ...
            for _ignored in range(columnCount - 1):
                subItem = (columnCount + subItem - 1) % columnCount if isShiftDown else (subItem + 1) % columnCount
    ...
    def EnableSorting(self):
        ...
        if self.smallImageList is None:
            self.SetImageLists()
        if not self.smallImageList.HasName(ObjectListView.NAME_DOWN_IMAGE) and self.smallImageList.GetSize(0) == (
            16,
            16,
        ):
    ...
    def StartCellEdit(self, rowIndex, subItemIndex):
        ...
        evt = OLVEvent.CellEditStartingEvent(
            self, rowIndex, subItemIndex, modelObject, cellValue, cellBounds, defaultEditor
        )
        self.GetEventHandler().ProcessEvent(evt)
        if evt.IsVetoed():
    ...
    def FinishCellEdit(self):
        ...
        evt = OLVEvent.CellEditFinishingEvent(
            self, rowIndex, subItemIndex, rowModel, self.cellEditor.GetValue(), self.cellEditor, False
        )
        self.GetEventHandler().ProcessEvent(evt)
    ...
    def CancelCellEdit(self):
        ...
        rowModel = self.GetObjectAt(rowIndex)
        evt = OLVEvent.CellEditFinishingEvent(
            self, rowIndex, subItemIndex, rowModel, self.cellEditor.GetValue(), self.cellEditor, True
        )
        self.GetEventHandler().ProcessEvent(evt)
...
class GroupListView(FastObjectListView):
    ...
    * putBlankLineBetweenGroups
    ...
    def SortGroups(self, groups=None, ascending=None):
        ...
        sortCol = self.GetSortColumn() if self.GetAlwaysGroupByColumn() else self.GetGroupByColumn()
...
class ColumnDefn:
    ...
    * width
    ...
    def GetAlignment(self):
        ...
        return {"l": wx.LIST_FORMAT_LEFT, "c": wx.LIST_FORMAT_CENTRE, "r": wx.LIST_FORMAT_RIGHT}.get(
            self.align[:1], wx.LIST_FORMAT_LEFT
        )
    ...
    def GetGroupTitle(self, group, useItemCount):
        ...
        if useItemCount:
            ...
            fmt = self.groupTitleSingleItem if objectCount == 1 else self.groupTitlePluralItems
